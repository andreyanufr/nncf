    ...
    def apply(
        ...
        for k, awq_data_item in track(awq_data.items(), description=description):
            ...
            if len(weight_data) != 1:  # not supported by the algorithm
                continue
            is_mergable = self._backend_entity.is_node_with_weights(merge_node, graph)
            ...
            print("Down proj", wp.node_with_weight.node_name, w_scale.min(), w_scale.max())
            print("Up proj", wp.node_with_weight.node_name, a_scale.min(), a_scale.max())
            ...
            if is_mergable:  # for MatMul->Multiply->MatMul pattern scale merged to first MatMul
