...
def create_nncf_qlinear(qweight, qzeros, scales, bits, group_size, bias, symmetric=True):
    if bits in (2, 3, 4):
        qweight = pack_uint4(qweight)

    qlinear = NNCFQLinear(
        qweight=qweight, qzeros=qzeros, scales=scales, bits=bits, group_size=group_size, sym=symmetric, bias=bias
    )
    return qlinear


def create_nncf_qlinear_dequantizer(qweight, qzeros, scales, bits, group_size, bias, symmetric=True):
    if bits in (2, 3, 4):
        qweight = pack_uint4(qweight)

    qlinear = NNCFQLinearDequantizer(
        qweight=qweight, qzeros=qzeros, scales=scales, bits=bits, group_size=group_size, sym=symmetric, bias=bias
    )
    return qlinear
...
if __name__ == "__main__":
    ...
    for shape, bits, group_size, symmetric in test_configs:
        ...
        try:
            qlinear = create_random_nncf_qlinear(shape, bits, group_size, symmetric)
